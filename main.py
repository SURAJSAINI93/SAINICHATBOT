...
API_ID = os.environ.get("12380656",) 
API_HASH = os.environ.get("d927c13beaaf5110f25c505b7c071273",) 
BOT_TOKEN = os.environ.get("7468224439:AAEGXiQeoK_iRjHwFFIu1In574UDV1sLGvs",) 
MONGO_URL = os.environ.get("mongodb+srv://Suraj43:Suraj43@suraj.0bg0zuc.mongodb.net/?retryWrites=true&w=majority",)
BOT_USERNAME = os.environ.get("RiyaM1bot",) 
UPDATE_CHNL = os.environ.get("UPDATE_CHNL","quizbys")
OWNER_USERNAME = os.environ.get("OWNER_USERNAME","attitude_boy43")
SUPPORT_GRP = os.environ.get("SUPPORT_GRP","https://t.me/+UXm1qdgiN243NWNl")
...
async def is_admins(chat_id: int):
    return [
        member.user.id
        async for member in SAINICHATBOT.get_chat_members(
...
@SAINICHATBOT.on_message(filters.incoming & filters.private, group=-1)
...
@SAINICHATBOT.on_message(filters.command(["start",f"start@{BOT_USERNAME}"]))
async def restart(client, m: Message):
        ...
        await accha.edit("https://t.me/+UXm1qdgiN243NWNl🍫✨")
        await asyncio.sleep(0.5)
        await accha.edit("https://t.me/+UXm1qdgiN243NWNl🍷✨")
        await asyncio.sleep(0.5)
        await accha.delete()
...
@SAINICHATBOT.on_callback_query()
...
@SAINICHATBOT.on_message(filters.command(["help", f"help@{BOT_USERNAME}"], prefixes=["","+", ".", "/", "-", "?", "$"]))
async def restart(client, message):
    hmm = await message.reply_photo(START_IMG,
                             caption= HELP_READ,
                        reply_markup= InlineKeyboardMarkup(HELP_BACK),
       )
@SAINICHATBOT.on_message(filters.command(['source', 'repo']))
async def source(bot, m):
    await m.reply_photo(START_IMG, caption=SOURCE_TEXT, reply_markup=SOURCE_BUTTONS, reply_to_message_id=m.id)
#  alive
@SAINICHATBOT.on_message(filters.command(["ping","alive"], prefixes=["","+", "/", "-", "?", "$", "&","."]))
...
async def vickai(client: Client, message: Message):
   ...
   if not message.reply_to_message:
       ...
       is_vick = vick.find_one({"chat_id": message.chat.id})
       if not is_vick:
           await SAINICHATBOT.send_chat_action(message.chat.id, ChatAction.TYPING)
           K = []  
   ...
   if message.reply_to_message:  
       ...
       vick = vickdb["VickDb"]["Vick"] 
       is_vick = vick.find_one({"chat_id": message.chat.id})    
       getme = await SAINICHATBOT.get_me()
       bot_id = getme.id                             
